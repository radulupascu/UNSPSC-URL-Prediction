...
# Assuming cluster_labels, product_embeddings, product_names, product_urls are predefined variables
...
fig = make_subplots(rows=1, cols=2, specs=[[{'type': 'scatter3d'}, {'type': 'scatter'}]], subplot_titles=('3D Visualization', '2D Visualization'))

# Custom HTML element for displaying hover information
hover_info = HTML()
display(hover_info)
...
for i in range(total_clusters):
    ...
    # Add 2D trace without adding to legend (no showlegend property)
    fig.add_trace(go.Scatter(
        ...
        customdata=hover_texts,
        legendgroup=f"group{i}",
        showlegend=False  # Ensure this is set to False to avoid duplicate legends
    ), row=1, col=2)
...
# Update the custom HTML on hover
def update_hover(trace, points, state):
    if points.point_inds:
        info = trace['customdata'][points.point_inds[0]]
        hover_info.value = f"<div style='width:600px; height:100px; text-align:center; border:1px solid black; padding:10px;'>{info}</div>"
    else:
        hover_info.value = ""

for trace in fig.data:
    trace.on_hover(update_hover)

fig.update_layout(width=1200, height=600, title='Visualization of Product Embeddings', margin=dict(l=0, r=0, b=0, t=40))
fig.show()
